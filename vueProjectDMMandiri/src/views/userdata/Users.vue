  ...
  <CRow>
    <CCol col="12" xl="12">
      <CCard>
          ...
          <CForm class="row g-3">
            <div class="col-auto mr-3">
              <div class="row g-3 align-items-center" style="height:50px;">
                <CButton @click="getModalAddForm()" v-if="AksesTambah" color="info" size="sm"> <svg width="14" height="22"
            ...
            <div class="col-auto">
              <div class="row g-3 align-items-center bg-info" style="height:50px;">
                <div class="col-auto">
                  <select class="form-select form-control" v-model="criteriaSearch">
                    <option value="nama">Berdasarkan Nama</option>
                    <option value="nama_peran">Berdasarkan Peran</option>
                  </select>
                </div>
                <div class="col-auto">
        ...
        <ModalAddEdit @open="submit()" :loadTheData="loadTheData"></ModalAddEdit>
        <ModalConfirm @open="submitConfirm()" :loadTheData="loadTheData"></ModalConfirm>
        <CCardBody>
          <CDataTable hover striped :loading="loading" :items="items" :fields="fields">
            ...
            <template #namaPengguna="{ item }">
              <td>
                <div v-if="item">
                  <div v-if="item">{{ item.idPengguna + " - " + item.namaPengguna + " - " + item.kodeCabang + " - " +
                    item.namaCabang }}</div>
                </div>
              </td>
            </template>
            <template #peran="{ item }">
              <td>
                <div v-if="item">
                  <div v-if="item">{{ item.kodePeran + " - " + item.namaPeran }}</div>
                </div>
              </td>
            </template>
            <template #id="{ item }">
...
export default {
  ...
  methods: {
    ...
    klikSearch() {
      this.MyfetchData(this.searchByName,this.criteriaSearch);
    },
    MyfetchData(search = "",criteriaSearch="") {
      fetch("http://localhost:8080/api/akuns/AkunData" + "?search=" + search+"&criteria="+criteriaSearch)
        .then(res => res.json())
        .then(
          (result) => {
            this.items = result;
    ...
    getModalAddForm() {
      ...
      storeDM.commit('changeTitleModal', { title: "Tambah Data Akun" })
      //store.dispatch({ type: 'CHANGE_STATE', payload: { titleModal:"Add New Data" } })

    },
    async getModalEditForm(item) {
      await storeDM.commit('changeFormData', { getData: item })
      await storeDM.commit('changeWarningModal', { modalState: true })
      storeDM.commit('changeTitleModal', { title: "Update Akun" })
      //store.dispatch({ type: 'CHANGE_STATE', payload: { titleModal:"Add New Data" } })
