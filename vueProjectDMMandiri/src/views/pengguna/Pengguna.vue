                ...
                <div class="col-auto">
                  <div class="col-auto">
                    ...
                    >
                      ...
                      <option value="alamat">Berdasarkan Alamat</option>
                    </select>
                  </div>
                </div>
                <div class="col-auto">
          ...
          >
            ...
            <template #cabang="{ item }">
              <td>
                <div v-if="item">
                  <CBadge color="primary">{{
                    primaryBadge(item.kodeCabang + " - " + item.namaCabang)
                  }}</CBadge>
                </div>
              </td>
            </template>
...
import ModalAddEdit from "@/views/ModalPengguna/ModalAddEdit";
import ModalConfirm from "@/views/ModalPengguna/ModalConfirm";
...
export default {
  ...
  methods: {
    ...
    MyfetchData(search = "", criteriaSearch = "") {
      fetch(
        "http://localhost:8080/api/penggunas/PenggunaData" +
          "?search=" +
          search +
          "&criteria=" +
          criteriaSearch
    ...
    getModalAddForm() {
      ...
      this.storeDM.commit("changeTitleModal", {
    ...
    async getModalConfirm(item) {
      await this.storeDM.commit("changeFormData", { getData: item });
      await this.storeDM.commit("changeConfirmModal", { modalState: true });
    }
  }
};
</script>
