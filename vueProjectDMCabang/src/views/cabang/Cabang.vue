...
import ModalAddEdit from "@/views/ModalCabang/ModalAddEdit";
import ModalConfirm from "@/views/ModalCabang/ModalConfirm";
...
export default {
  ...
  methods: {
    ...
    MyfetchData(search = "") {
      fetch(
        "http://localhost:8080/api/cabangs/CabangData" + "?search=" + search
      )
        .then((res) => res.json())
        .then((result) => {
          0;
    ...
    getModalAddForm() {
      ...
      this.storeDM.commit("changeTitleModal", { title: "Tambah Cabang Baru" });
    },
    async getModalEditForm(item) {
      ...
      await this.storeDM.commit("changeTitleModal", { title: "Update Cabang" });
    },
