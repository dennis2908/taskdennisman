...
const storeDM = new Vuex.Store({
	 ...
	 mutations: {
				...
				EmptyFormData(state){
					state.formData.id = "";
					state.formData.namaLayar = "";
					state.formData.kodeLayar = ""
				},
				changeFormData(state, { getData }){
					console.log(55555,getData);
					state.formData.id = getData.id;
					state.formData.namaLayar = getData.namaLayar;
					state.formData.kodeLayar = getData.kodeLayar
				}
            },
})

const storeLayar = createStore(reducer)

//const store} = useContext(ReactReduxContext)

export {storeLayar,storeDM}